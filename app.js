 ...
 * @returns {boolean} true se l'utente ha ruolo admin o tesoriere
 */
...
async function handleLogin(event) {
    ...
    try {
        ...
        showMessage('loginError', 'Errore di connessione al server');
...
async function loadDashboard() {
    try {
        const response = await fetch(`${API_BASE}/api/riepilogo?anno=${new Date().getFullYear()}`, {
            headers: {
                'Authorization': `Bearer ${authToken}`
            }
        });
...
async function loadAllTransactions() {
    ...
    try {
        const response = await fetch(`${API_BASE}/api/transazioni?limit=1000`, {
            headers: {
                'Authorization': `Bearer ${authToken}`
            }
        });
...
async function editTransaction(id) {
    ...
    try {
        const response = await fetch(`${API_BASE}/api/transazioni/${id}`, {
            headers: {
                'Authorization': `Bearer ${authToken}`
            }
        });
...
async function handleAddTransaction(event) {
    ...
    try {
        const url = isEditingTransaction 
            ? `${API_BASE}/api/transazioni/${editingTransactionId}`
            : `${API_BASE}/api/transazioni`;
...
async function loadCategories() {
    try {
        ...
        const entrateResponse = await fetch(`${API_BASE}/api/categorie/entrate`);
        if (entrateResponse.ok) {
            categorieEntrate = await entrateResponse.json();
        }
        
        // Carica categorie uscite
        const usciteResponse = await fetch(`${API_BASE}/api/categorie/uscite`);
...
async function deleteCategory(tipo, id) {
    ...
    try {
        const response = await fetch(`${API_BASE}/api/categorie/${tipo}/${id}`, {
            method: 'DELETE',
            headers: {
                'Authorization': `Bearer ${authToken}`
            }
...
async function handleAddCategory(event) {
    ...
    try {
        const url = isEditingCategory 
            ? `${API_BASE}/api/categorie/${editingCategoryType}/${editingCategoryId}`
            : `${API_BASE}/api/categorie/${tipo}`;
