...
async function caricaDatiPerFiltri() {
    ...
    if (!authToken) {
        console.warn('Token di autenticazione mancante per i filtri');
        return;
    }

    try {
        // Carica tutte le transazioni (limite alto per avere tutto)
        const responseTransazioni = await fetch(`${API_BASE}/api/transazioni?limit=10000`, {
...
async function caricaCategoriePerFiltri() {
    try {
        const [responseEntrate, responseUscite] = await Promise.all([
            fetch(`${API_BASE}/api/categorie/entrate`),
            fetch(`${API_BASE}/api/categorie/uscite`)
        ]);
...
function integrazioneConAppPrincipale() {
    // Aspetta che le variabili globali dell'app principale siano disponibili
    if (typeof authToken !== 'undefined' && authToken) {
        inizializzaSistemaFiltri();
    } else {
        // Riprova tra 1 secondo se l'app non è ancora pronta
        setTimeout(integrazioneConAppPrincipale, 1000);
    }
}
